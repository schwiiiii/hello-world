<template>
  <div class="marketing-message h-64" :style="shellBinder">
    ...
    >
      ...
      <div class="-m-1">
        <button class="bg-white text-black py-1 px-3 m-1" @click="prevSlide">
          Previous
        </button>
        <button class="bg-white text-black py-1 px-3 m-1" @click="nextSlide">
          Next
        </button>
      </div>
    </div>
...
export default {
  ...
  computed: {
    ...
    currentBGSource() {
      return require(`@/assets/marketing-message/${
        this.data.slides[this.activeSlide].image
      }`);
    },
    shellBinder() {
      return {
  ...
  methods: {
    prevSlide() {
      this.activeSlide =
        this.activeSlide == 0 ? this.slideCount - 1 : this.activeSlide - 1;
    },
    nextSlide() {
      this.activeSlide =
        this.activeSlide == this.slideCount - 1 ? 0 : this.activeSlide + 1;
    },
